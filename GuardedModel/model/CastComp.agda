...
--     codeMeet (c1 |wf| wf1) (c2 |wf| wf2) reflp reflp | .(HStatic _)  | .(HStatic _)  | HNeq x = C℧ |wf| IWF℧
--     -- If either is ⁇, then the meet is just the other code
--     codeMeet (C⁇ |wf| wf1) (c2 |wf| wf2) reflp reflp | h1  | h2  | H⁇L reflp x₁ = c2 |wf| wf2
--     codeMeet (c1 |wf| wf1) (C⁇ |wf| wf2) reflp reflp | .(HStatic _)  | h2  | H⁇R reflp = c1 |wf| wf1
...
-- -- --       → { pfc1 : (wfSize c)  ≡p cSize }
-- -- --       → {{ pfc2 : O1  ≡p cSize }}
-- -- --       → { (wfElSize c x)  ≡p vSize }
-- -- --       → { (wfElSize c y)  ≡p vSize }
-- -- --       → LÆ (wfEl c)
...
-- -- --       →  (pfc1 : O1 ≡p cSize)
-- -- --       →  (pfc2 : (wfSize c) ≡p cSize)
-- -- --       → codeHead (code c) ≡p HStatic h
-- -- --       → (x : wfEl c)
-- -- --       → (pfv1 : wfElSize c x ≡p vSize)
-- -- --       →  (pfv2 : O1 ≡p vSize)
-- -- --       → LÆ (germ h ℓ)
...
-- -- --       → codeHead (code c) ≡p HStatic h
-- -- --       → (x : El {ℓ} C⁇)
-- -- --       →  (pfv1 : elSize C⁇ x ≡p vSize)
-- -- --       →  (pfv2 : O1 ≡p vSize)
-- -- --       → LÆ (wfEl c)

-- -- --     toDataGerm : ∀ {{_ : Æ}} {cI : ℂ ℓ} (tyCtor : CName) (D : DName tyCtor → ℂDesc cI C𝟙 )
-- -- --       → {i : ApproxEl cI}
-- -- --       → {@(tactic default (reflp {A = Ord} {cSize})) pfc1 :  (codeSize (Cμ tyCtor cI D i))  ≡p cSize }
-- -- --       → {@(tactic default (reflp {A = Ord} {cSize})) pfc2 :  (dataGermDescSize ℓ tyCtor)  ≡p cSize }
-- -- --       → (x : ℂμ tyCtor D i)
-- -- --       → {@(tactic default (reflp {A = Ord} {vSize})) pfv1 : elSize (Cμ tyCtor cI D i) (transport ℂμW x)  ≡p vSize }
-- -- --       → {@(tactic default (reflp {A = Ord} {vSize})) pfv2 : elSize (Cμ tyCtor cI D i) (transport ℂμW x)  ≡p vSize }
...
-- -- --       → {@(tactic default (reflp {A = Ord} {vSize})) pfv1 : O1  ≡p vSize }
-- -- --       → {@(tactic default (reflp {A = Ord} {vSize})) pfv2 : O1  ≡p vSize }
-- -- --       → (ℂμ tyCtor D i)


-- -- --     cast : ∀ {{_ : Æ}}
-- -- --       → (csource cdest : ℂwf ℓ)
-- -- --       → (pfc1 :(wfSize cdest)  ≡p cSize)
-- -- --       → ( pfc2 :  (wfSize csource) ≡p cSize)
-- -- --       →  (x : wfEl csource)
-- -- --       → (pfv1 : wfElSize csource x ≡p vSize)
-- -- --       → (pfv2 : O1 ≡p vSize)
-- -- --       -> LÆ ( wfEl cdest)
