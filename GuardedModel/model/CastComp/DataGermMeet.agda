...
--     xrec ← oDataGermMeet (self (<VSize reflc lt')) x1 x2 reflp
...
-- germFIndMeet {posNoCode = pnc} {cpf = cpf} (GUnk (A+ , A-) D) (GUnkCode c+ c- iso+ iso- isCode)  b+ b- (FC c1 r1) (FC c2 r2) lt = do
...
--       ltRec =
--         ≤ₛ-sucMono (smax-mono
--           (≤↑ _ ≤⨟ ≤ₛ-sucMono ({!!} ≤⨟ ≤ₛ-cocone {!!}))
--           {!!})
--     ⁇result ← oMeet (self (<VSize reflc (<≤ ltRec lt))) C⁇ (f1 a+) (f2 a+) cpf reflp
--     pure (⁇ToW ⁇result)
...
--             {!!}
--             {!!})
--       ⁇result ← oMeet (self (<VSize reflc (<≤ lt' lt))) {{æ = Approx}} C⁇ (▹f1 (a+ , Iso.inv (iso- b+ (fst _) b-) tt*)) (▹f2 (a+ , Iso.inv (iso- b+ _ b-) tt*)) {!!} reflp
--       Now {!!}})
--     -- Exact case, use guarded recursion
--     (λ { eq → do
--       gSelf ← Lself eq
--       ⁇result ← oMeet gSelf C⁇ (▹f1 (a+ , a-)) (▹f2 (a+ , a-)) cpf reflp
--       pure (⁇ToW ⁇result) })
--     --
--     -- pure {!!}
...
--         r
--       ; (Rest x) → rret (inr x) }) )))
-- germFIndMeet {posNoCode = pnc} {cpf = cpf} {isPos = reflp} (GArg (A+ , A-) D) (GArgCode c+ c- iso+ iso- isCode)  b+ b-
...
--       gSelf1 ← Lself eq
--       gSelf2 ← Lself eq
--       (a-1Cast , lt1) ← oCast gSelf1 (c- b+ a+1 b-) (c- b+ (Iso.inv (iso+ b+) ca+) b-) (pnc reflp) ca-1 reflp
--       (a-2Cast , lt2) ← oCast gSelf2 (c- b+ a+2 b-) (c- b+ (Iso.inv (iso+ b+) ca+) b-) (pnc reflp) ca-2 reflp
--       gSelf3 ← Lself eq
--       ret ← oMeet gSelf3 (c- b+ (Iso.inv (iso+ b+) ca+) b-) a-1Cast a-2Cast (pnc reflp) reflp
--       pure (Iso.inv (iso- b+ (Iso.inv (iso+ b+) ca+) b-) (next ret))

--   -- Compute the meet of the negative fields
--   -- Compute the meet of the rest
--   (FC crec rrec) ← germFIndMeet {isPos = reflp} D isCode (b+ , Iso.inv (iso+ b+) ca+) (b- , ca-) (FC {!!} {!!}) {!!} {!!}
--   -- Put it all together
--   pure (FC (
--     (Iso.inv (iso+ b+) ca+
--     , {!!}) ,
