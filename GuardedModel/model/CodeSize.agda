...
--   → □ _ (λ _ → Size) (tt , cs)
--   → Size
germIndSize : ∀ {{ æ : Æ }} {ℓ} (tyCtor : CName) →  DataGerm ℓ tyCtor → Size
...
germIndSize {ℓ} tyCtor x = wRec {I = Maybe CName} {X = λ _ → Size} {!!} (λ _ → (S1 , S1)) x
--(wRecArg tyCtor Size (λ d cd φ → S↑ {!!}) S1 S1 {!!})
