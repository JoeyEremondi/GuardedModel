...
germFIndSize : ∀ {{æ : Æ}} {ℓ} {B} (tyCtor : CName) → (D : GermCtor B)
  ...
  → □ _ (λ _ → Ord) (tt , cs)
  → Ord
guradedCodeSize : ∀ {{æ : Æ}} {ℓ} {A} → IsGuardedCode ℓ A → Ord
...
germFIndSize tyCtor (GArg A D) (GArgCode x pf) b (FC (a , com) k unk) φ = O↑ (germFIndSize tyCtor D pf (b , a) (FC com k unk) φ)
germFIndSize tyCtor (GHRec A D) (GHRecCode x pf) b (FC com k unk) φ = {!O↑ (OLim ? ?)!}
germFIndSize tyCtor (GRec D) (GRecCode pf) b (FC com k unk) φ =
  O↑ (omax (φ (Rec tt)) (germFIndSize tyCtor D pf b (FC com (λ r → k (Rest r)) unk) (λ r → φ (Rest r))))
germFIndSize tyCtor (GUnk A D) (GUnkCode c x pf) b (FC com k unk) φ = {!!}
