...
DataGermIsCode : ∀ (ℓ : ℕ) {{æ : Æ}}  {B : Set} → GermCtor B → B → Set2
DataGermIsCode ℓ GEnd b = Unit*
DataGermIsCode ℓ (GArg A D) b = (Σ[ c+ ∈ ℂ ℓ ] (Σ[ c- ∈ (El c+ → ℂ ℓ) ] Iso (A b) (Σ[ x ∈ El c+ ] (▹ (El (c- x)))) ) ) × ∀ a → DataGermIsCode ℓ D (b , a)
DataGermIsCode ℓ (GHRec A D) b = (Σ[ c+ ∈ ℂ ℓ ] (Σ[ c- ∈ (El c+ → ℂ ℓ) ] Iso (A b) (Σ[ x ∈ El c+ ] (▹ (El (c- x)))) ) ) × DataGermIsCode ℓ D b
DataGermIsCode ℓ (GRec D) b = DataGermIsCode ℓ D b
DataGermIsCode ℓ (GUnk A D) b = (Σ[ c+ ∈ ℂ ℓ ] (Σ[ c- ∈ (El c+ → ℂ ℓ) ] Iso (A b) (Σ[ x ∈ El c+ ] (▹ (El (c- x)))) ) ) × DataGermIsCode ℓ D b
