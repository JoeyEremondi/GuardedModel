...
module InductiveCodes {{_ : DataTypes}} {{_ : DataGerms}} {{æ : Æ}} where
...
data ℂDescEl' {ℓ} (cI : ℂ ℓ) (X : ApproxEl cI → Set) :  ℂDesc cI → ApproxEl cI → Set where
  ElEnd : ∀ { i} j → i ≅ j →  ℂDescEl' cI X (CEnd j) i
  ElArg : ∀ {cA D i} → (a : Approxed (El cA) ) →  ℂDescEl' cI X  (D (approx a)) i → ℂDescEl' cI X  (CArg cA D) i
  ElRec : ∀ {i} {j : ApproxEl cI} {D : ℂDesc cI} →
    X j → ℂDescEl' cI X  D i → ℂDescEl' cI X  (CRec j D) i
  ElHRec : ∀ {i} {cA : ℂ ℓ} {j : (ApproxEl cA) → ApproxEl cI} {D : (ApproxEl cA) → ℂDesc cI}
    → ((a : Approxed (El cA) {{æ}}) → (X (j (approx a))))
    → ((a : Approxed (El cA) {{æ}}) → (ℂDescEl' cI X  (D (approx a)) i))
...
ℂDescEl : ∀  {ℓ} {cI : ℂ ℓ} → ℂDesc cI → (ApproxEl cI → Set) → ApproxEl cI → Set
...
data ℂμ {ℓ} {cI : ℂ ℓ} (tyCtor : CName) (D : DName tyCtor → ℂDesc cI) (i : ApproxEl cI)  : Set where
...
WArg : ∀ {ℓ} {cI : ℂ ℓ} {tyCtor : CName} (D : DName tyCtor → ℂDesc cI) → ApproxEl cI → Set
...
ℂμW : ∀ {ℓ} {cI : ℂ ℓ}  {tyCtor : CName} {D : DName tyCtor → ℂDesc cI} {i : ApproxEl cI}  →
...
fromCElCommand : ∀ {ℓ} {cI : ℂ ℓ} (D : ℂDesc cI) {i : ApproxEl cI} {X : ApproxEl cI → Set}
  → ℂDescEl  D X i
  → CommandD D i
fromCElCommand .(CEnd j) (ElEnd j x) = x
fromCElCommand (CArg _ D) (ElArg a x) = a , fromCElCommand (D (approx a)) x
fromCElCommand (CRec _ D) (ElRec x x₁) = fromCElCommand D x₁
fromCElCommand (CHRec c j D) (ElHRec x f) a = fromCElCommand (D (approx a)) (f a)
-- fromCElCommand (CHGuard c D1 D2) (ElHGuard f x) = (λ a → fromCElCommand D1 (f a)) , (fromCElCommand D2 x)



fromCElF : ∀ {ℓ} {cI : ℂ ℓ} (D : ℂDesc cI) {X : ApproxEl cI → Set} {i : ApproxEl cI}
  → (x : ℂDescEl  D X i)
  → (r : ResponseD D (fromCElCommand D x))
      → X (inextD D (fromCElCommand D x) r)
fromCElF (CArg c D) (ElArg a x) r = fromCElF (D (approx a)) x r
...
fromCElF (CHRec c i D) (ElHRec f1 f2) (Rest (a , r)) = fromCElF (D (approx a)) (f2 a) r
...
fromCμ : ∀ {ℓ} {cI : ℂ ℓ} {tyCtor : CName} {D : DName tyCtor → ℂDesc cI} {i : ApproxEl cI}
  → ℂμ tyCtor D i
  → WArg D i
fromCEl : ∀ {ℓ} {cI : ℂ ℓ} {tyCtor : CName} (D : ℂDesc cI) (E : DName tyCtor → ℂDesc cI) {i : ApproxEl cI}
...
fromCEl (CArg c D) E (ElArg a x) r = fromCEl (D (approx a)) E x r
...
fromCEl (CHRec c i D) E (ElHRec f1 f2) (Rest (a , r)) = fromCEl (D (approx a)) E (f2 a) r
...
toCEl :
  ∀ {ℓ} {cI : ℂ ℓ} {tyCtor : CName} (D : ℂDesc cI) (E : DName tyCtor → ℂDesc cI) {ix : ApproxEl cI} →
  (com : CommandD D ix) →
  (k : (r : ResponseD D com ) →
...
toCμ : ∀ {ℓ} {cI : ℂ ℓ} {tyCtor : CName} (D : DName tyCtor → ℂDesc cI) {ix : ApproxEl cI}
  → (x : WArg D ix)
  → ℂμ tyCtor D ix
...
toCEl (CArg c D) E (a , com) k φ = ElArg a (toCEl (D (approx a)) E com k φ)
toCEl (CRec j D) E com k φ = ElRec (φ (Rec tt)) (toCEl D E com (λ r → k (Rest r)) λ r → φ (Rest r))
toCEl (CHRec c j D) E com k φ = ElHRec (λ a → φ (Rec a)) (λ a → toCEl (D (approx a)) E (com a) (λ r → k (Rest (a , r))) λ r → φ (Rest (a , r)))
...
toCElF :
  ∀ {ℓ} {cI : ℂ ℓ} {X : ApproxEl cI → Set} (D : ℂDesc cI)  {ix : ApproxEl cI} →
  (com : CommandD D ix) →
  (k : (r : ResponseD D com ) → X (inextD D com r))
  → (ℂDescEl  D X ix)
toCElF (CEnd i) wit k = ElEnd i wit
toCElF (CArg c D) (a , com) k = ElArg a (toCElF (D (approx a)) com k)
toCElF (CRec j D) com k = ElRec (k (Rec tt)) (toCElF D com (λ r → k (Rest r)))
toCElF (CHRec c j D) com k = ElHRec (λ a → k (Rec a)) (λ a → toCElF (D (approx a)) (com a) (λ r → k (Rest (a , r))))
...
fromToCElCommand (CArg c D) E (a , com) k   = ΣPathP (refl , fromToCElCommand (D (approx a)) E com k  )
fromToCElCommand (CRec j D) E com k   = fromToCElCommand D E com (λ r → k (Rest r))
fromToCElCommand (CHRec c j D) E com k   = funExt λ a → fromToCElCommand (D (approx a)) E (com a) (λ r → k (Rest (a , r)))
...
fromToCEl (CodeModule.CArg c D) E (a , com) k  φ  = fromToCEl (D (approx a)) E com k φ
...
fromToCEl (CodeModule.CHRec c j D) E com k φ 𝕚 (Rest (a , r)) = fromToCEl (D (approx a)) E (com a) (λ r → k (Rest (a , r))) (λ r → φ (Rest (a , r))) 𝕚 r
...
fromToCμ :  ∀ {ℓ} {cI : ℂ ℓ} {tyCtor : CName} (D : DName tyCtor → ℂDesc cI) {ix : ApproxEl cI}
  → (x : WArg D ix)
  → fromCμ (toCμ D x) ≡ x
fromToCμ {cI = cI} D = wInd
  (λ(ix , x) → fromCμ (toCμ D x) ≡ x) helper refl refl
  where
    helper : ∀ {i : ApproxEl cI} (cs : FContainer (Arg (λ d → interpDesc (D d))) (WArg D) Unit i)  →  (φ : _) → fromCμ (toCμ D (Wsup cs)) ≡ Wsup cs
...
toFromCμ : ∀ {ℓ} {cI : ℂ ℓ} {tyCtor : CName} {D : DName tyCtor → ℂDesc cI} {i : ApproxEl cI}
  → (x : ℂμ tyCtor D i)
  → toCμ D (fromCμ x) ≡ x
toFromCEl : ∀ {ℓ} {cI : ℂ ℓ} {tyCtor : CName} (D : ℂDesc cI) (E : DName tyCtor → ℂDesc cI) {i : ApproxEl cI}
  → (x : ℂDescEl  D (ℂμ tyCtor E) i)
...
toFromCEl (CArg c D) E (ElArg a x) = cong (ElArg a) (toFromCEl (D (approx a)) E x)
toFromCEl (CRec j D) E (ElRec x x₁) = cong₂ ElRec (toFromCμ x) (toFromCEl D E x₁)
toFromCEl (CHRec c j D) E (ElHRec x x₁) = cong₂ ElHRec (funExt (λ a → toFromCμ (x a))) (funExt λ a → toFromCEl (D (approx a)) E (x₁ a))
