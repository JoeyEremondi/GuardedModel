...
-- Telescopes for the arguments of data germs, separating positive and negative ⁇ occurrences

data +-Set : Set1
+Set : +-Set → Set
-Set  : (B+- : +-Set) → (+Set B+-) -> Set -> Set

data +-Set where
  +-Nil : +-Set
  +-Cons : (B+- : +-Set) → (A+ : +Set B+- → Set ) → (A-RHS : (Self : Set) → (b : +Set B+-)  → (A+ b) → -Set B+- b Self → Self → Set) → +-Set

-- The types encoded by +-Types
-- Positive is just a type
-- Negative is parameterized over our (Guarded ) reference to ⁇
+Set +-Nil = 𝟙
+Set (+-Cons B+- A+ A-RHS) = Σ[ b+ ∈ +Set B+- ] (A+ b+)

-Set +-Nil B+ Self = 𝟙
-Set (+-Cons B+- A+ A-RHS) (b+ , a+) Self = Σ[ b- ∈ -Set B+- b+ Self ] ((x : Self) → A-RHS Self b+ a+ b- x)


record +-Ext (B+- : +-Set)  : Set1 where
  -- constructor +-set
  field
    A+set : +Set B+- → Set
    A-RHS : (Self : Set) → (b : +Set B+-)  → (A+set b) → -Set B+- b Self → Self → Set
  ...
  A-set : (Self : Set) → (b : +Set B+-) → (a+ : A+set b) → (b- : -Set B+- b Self) → Set
...
+-Cons' : (B+- : +-Set) → +-Ext B+- → +-Set
+-Cons' B+- A+- = +-Cons B+- (A+set A+-) (A-RHS A+-)
...
data GermCtor : (B+- : +-Set) → IndSig → Set1 where
  ...
  GArg : ∀ {B+- sig} → (A+- : +-Ext B+- ) → (D : GermCtor (+-Cons' B+- A+-) sig  ) → GermCtor  B+- (SigA sig)
  GHRec : ∀ {B+- sig} → (A : +-Ext B+- ) → (D : GermCtor  B+- sig) → GermCtor  B+- (SigHR sig)
  ...
  -- -- TODO: is this right?
  GUnk : ∀ {B+- sig} → (A : +-Ext B+- ) → (D : GermCtor  B+- sig) → GermCtor  B+- (SigA sig)

GermCommand : ∀  {B+- sig Self} → GermCtor B+- sig → (b : +Set B+-) → (-Set B+- b Self) → Set
GermCommand GEnd b+ b- = Unit
GermCommand {Self = Self} (GArg A+A- D) b+ b- = Σ[ a+- ∈  (Σ[ a+ ∈ (A+set A+A- b+) ] (A-set A+A- Self b+ a+ b-)) ] GermCommand D (b+ , (fst a+-)) (b- , (snd a+-))
...
GermResponse : ∀ {{æ : Æ}} {B+- sig Self} → (D : GermCtor B+- sig) → (b+ : +Set B+-) → (b- : -Set B+- b+ Self) → GermCommand D b+ b- → Set
GermResponse {B+}{ B- } GEnd b+ b- com = 𝟘
GermResponse {B+}{ B- } (GArg A D) b+ b- ((a+ , a-) , com) = GermResponse D (b+ , a+) (b- , a-) com
GermResponse {B+ }{B- } {Self = Self} (GHRec A+A- D) b+ b- com =
  -- We have two functions, one for just the positive part, and one for the negative part
  Rec⇒  ( (A+set A+A- b+) ⊎ (Σ[ a+ ∈ (A+set A+A- b+) ]  (A-set A+A- Self b+  a+ b-)))
...
GermResponseUnk : ∀ {{æ : Æ}} {B+- sig Self} → (D : GermCtor B+- sig) → (b+ : +Set B+-) → (b- : -Set B+- b+ Self) → GermCommand D b+ b- → Set
...
GermResponseUnk {Self = Self} (GUnk A+A- D) b+ b- com =
  Rec⇒ ( (A+set A+A- b+) ⊎ (Σ[ a+ ∈ (A+set A+A- b+) ] (A-set A+A- Self b+ a+ b-)))
...
interpGermCtor' : ∀ {{æ : Æ}} {A} {B+- sig Self} → GermCtor B+- sig → (b : +Set B+-) → -Set B+- b Self → Container (Maybe A)
...
interpGermCtor : ∀ {{æ : Æ}} {A} {sig} → GermCtor +-Nil sig → Set → Container (Maybe A)
interpGermCtor D Self = interpGermCtor' {Self = Self} D tt tt --interpGermCtor' D tt tt
-- --
-- -- fs qq

toApproxNeg : ∀ B+- {  ▹Self b+} → -Set B+- b+ (▹⁇Ty ▹Self) → -Set B+- b+ 𝟙
toApproxNeg +-Nil f = tt
toApproxNeg (+-Cons B+- A+ A-RHS) (fB , fA) = toApproxNeg B+- fB , λ _ → {!fA!}

-- toExactNeg : ∀ {B+- sig ▹Self b+}  → -Set B+- b+ 𝟙 → -Set B+- b+ (▹⁇Ty ▹Self)



-- toApproxCommand : ∀  {B+- sig Self} → (D : GermCtor B+- sig) → (b : +Set B+-) → (b- : -Set B+- b Self) → GermCommand {Self = Self} D b b- → GermCommand {Self = 𝟙} D b {!!}
...
--   → (▹Self : ▹ ⁇Self)
--   → (DescFor : (tyCtor : Fin numTypes) → (ctor : Fin (numCtors tyCtor)) → GermCtor +-Nil (sigs tyCtor ctor) )
...
--     preDataGerm : ℕ → (c : CName) → ((Self : Set) → (d : DName c) → GermCtor +-Nil (indSkeleton c d) )
