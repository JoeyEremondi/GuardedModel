...
underLim {c = c} o f all = ≤o-trans (≤o-cocone {c = c} (λ _ → o) (℧Approx c) (≤o-refl o)) (≤o-limiting (λ _ → o) (λ k → ≤o-cocone f k (all k)))
...
abstract
  omax : Ord → Ord → Ord
  omax o1 o2 = OLim {ℓ = 0} C𝔹 λ a → if a then o1 else o2
  ...
  omax-≤L {o1} {o2}   =
    ≤o-cocone _ true (≤o-refl _)

  omax-≤R : ∀ {o1 o2} → o2 ≤o omax o1 o2
  omax-≤R {o1} {o2}   =
    ≤o-cocone _ false (≤o-refl _)
  ...
  data UBView : Ord → Ord → Set where
    ...
    UB-LimL : ∀  {ℓ} {c : ℂ ℓ} {f : ApproxEl c → Ord} { o1 o2} → (¬ (o1 ≡p OZ)) → (¬ (o2 ≡p OZ)) → ((o1 ≡p OLim c f) ⊎ (o2 ≡p OLim c f)) → UBView o1 o2
...
+o-≤-R (OLim c f) (OLim c₁ f₁) = ≤o-limiting f₁ (λ k → ≤o-cocone (λ a → OLim c₁ (λ a₁ → f a +o f₁ a₁)) (℧Approx c) (≤o-cocone _ k (+o-≤-R (f _) (f₁ k))))
...
limLT : ∀ {{_ : Æ}} {ℓ} {c : ℂ ℓ}  {f : ApproxEl c → Ord} { x} → f x <o O↑ (OLim c f)
limLT {c = c} {f} {x} = ≤o-sucMono (≤o-cocone f x (≤o-refl (f x)))

limMaxLT-R : ∀ {{_ : Æ}} {o} {ℓ} {c : ℂ ℓ} {f : ApproxEl c → Ord} { x} → f x <o O↑ (omax o (OLim c f))
limMaxLT-R {f = f} {x = x} = ≤o-sucMono (≤o-trans (≤o-cocone f x (≤o-refl (f x))) omax-≤R)

maxInLimGen-L : ∀ {{_ : Æ}} {ℓ} {c : ℂ ℓ} {f1 f2 : ApproxEl c → Ord}  → OLim c f1 <o O↑ (OLim c λ a → omax (f1 a) (f2 a))
maxInLimGen-L {c = c} {f1} {f2} = ≤o-sucMono (extLim f1 (λ a → omax (f1 a) (f2 a)) (λ k → omax-≤L))

maxInLimGen-R : ∀ {{_ : Æ}} {ℓ} {c : ℂ ℓ} {f1 f2 : ApproxEl c → Ord}  → OLim c f2 <o O↑ (OLim c λ a → omax (f1 a) (f2 a))
maxInLimGen-R {c = c} {f1} {f2} = ≤o-sucMono (≤o-limiting f2 λ a → (≤o-cocone (λ a₁ → omax (f1 a₁) (f2 a₁)) a omax-≤R))

maxInLimApp-L : ∀ {{_ : Æ}} {ℓ} {c : ℂ ℓ} {f1 f2 : ApproxEl c → Ord} {x}  → f1 x <o O↑ (OLim c λ a → omax (f1 a) (f2 a))
...
maxInLimApp-R : ∀ {{_ : Æ}} {ℓ} {c : ℂ ℓ} {f1 f2 : ApproxEl c → Ord} {x}  → f2 x <o O↑ (OLim c λ a → omax (f1 a) (f2 a))
