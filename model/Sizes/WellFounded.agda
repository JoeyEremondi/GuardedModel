...
record LexOrder
  {ℓa ℓ'a} {A : Type ℓa} {_<a_ : A → A → Type ℓ'a}
  {ℓb ℓ'b} {B : Type ℓb} {_<b_ : B → B → Type ℓ'b} : Type (ℓ-max ℓa (ℓ-max ℓ'a (ℓ-max ℓb ℓ'b)))  where
  constructor Lex
  field
    wfa : WellFounded _<a_
    wfb : WellFounded _<b_

  data _<Lex_ : (A × B) → (A × B) → Type (ℓa ⊔ ℓb ⊔ ℓ'a ⊔ ℓ'b) where
    <LexL : ∀ {a a' b b'} → a <a a' → (a , b) <Lex (a' , b')
    <LexR : ∀ {a a' b b'} → a ≡ a' → b <b b' → (a , b) <Lex (a' , b')

  LexWellFounded : WellFounded _<Lex_
  LexWellFounded (a , b) = acc (helper (wfa a) (wfb b))
    where
  ...
  ∥LexWellFounded∥ : WellFounded (λ x y → ∥ x <Lex y ∥₁)
  ∥LexWellFounded∥ = ∥WellFounded∥ LexWellFounded
