...
data ℂDescEl' {ℓ} (X : Set) : {sig : IndSig} (cB : ℂ ℓ) →  ℂDesc cB sig → ApproxEl cB → Set where
  ...
  ElRec : ∀ {cB n b sig} {c : ApproxEl cB → ℂ ℓ} {D : ℂDesc cB sig} →
    ((x : El (c b)) → X ) → ℂDescEl' X cB D b → ℂDescEl' X cB  (CRec c D _ reflp) b
...
-- fromCElCommand (CRec c D _ _) (ElRec f x) = fromCElCommand D x
...
-- fromCElF (CRec c D _ _) (ElRec f1 f2) (Rec a) =  f1 a
-- fromCElF (CRec c D _ _) (ElRec f1 f2) (Rest r) =  fromCElF D f2 r --fromCElF (D (approx a)) (f2 a) r
...
-- fromCEl (CRec c D _ _) E (ElRec f1 f2) (Rec a) = fromCμ (f1 a)
-- fromCEl (CRec c D _ _) E (ElRec f1 f2) (Rest r) = fromCEl D E f2 r
...
-- toCEl (CRec c D _ reflp) E com k φ = ElRec (λ a → φ (Rec a)) (toCEl D E com (λ r → k (Rest r)) λ r → φ (Rest r))
...
-- toCElF (CRec c D _ reflp) com k = ElRec (λ a → k (Rec a)) (toCElF D com (λ r → k (Rest r)))
...
-- fromToCElCommand (CRec c D _ reflp) E com k   = fromToCElCommand D E com (λ r → k (Rest r))
